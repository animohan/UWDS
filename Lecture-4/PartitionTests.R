...
# TestBasic(PartitionWrong)
# TestBasic(PartitionFast)
# TestBasic(PartitionExact)
# TestWrong(PartitionWrong)
# TestApprox(PartitionFast)
# TestRandom(PartitionFast)
# TestRandom(PartitionExact)
# TestExact(PartitionWrong)
# TestExact(PartitionExact)
# # Planned Failures.  Expect: "Test found problems"
# TestWrong(PartitionExact)
# TestWrong(PartitionFast)
# TestApprox(PartitionWrong)
# TestApprox(PartitionExact)
# TestRandom(PartitionWrong)
# TestExact(PartitionFast)
...
TestExact <- function(Partition = PartitionWrong, numberOfTries = 200)
{
  success <- "Test could not complete"
  testsCompleted <- 0
  for (TryNumber in 1:numberOfTries)
  {
    ...
    fractionOfTest <- getFractionOfTest()
    basicTest <- Partition(dataSet, fractionOfTest)
    if (abs(fractionOfTest*nrow(dataSet) - nrow(basicTest$testingData)) <= 1.0)
    {
      testsCompleted <- testsCompleted + 1
    } else
    {
      success <- "Unexpected number of test rows"
    } # if else
  } # for
  if ((testsCompleted == numberOfTries) && (success == "Test could not complete"))
  {
    success <- "Tests have correct results"
  } else if ((testsCompleted < numberOfTries) && (success == "Unexpected number of test rows"))
  {
    success <- paste("Test found problems.  Only ", testsCompleted, " completed tests; ", success)
  } else
  {
    success <- paste("Test failed. ", testsCompleted, " completed tests; ", success)
  } # 2X if else
  return(success)
} # TestExact
